...
Проект представляет собой клиент-серверную архитектуру для простой биржи торговли долларами (USD) за рубли (RUB). Клиенты могут подавать заявки на покупку и продажу валюты, и сервер будет обрабатывать эти заявки, заключая сделки, если цены на покупку и продажу пересекаются.

...
BOOST_AUTO_TEST_CASE(TestMatchOrder3) 
{
    Core& core = GetCore();
    std::string EveBuyerId = core.RegisterNewUser("Eve");
    std::string FrankBuyerId = core.RegisterNewUser("Frank");
    ...
    core.SubmitOrder(EveBuyerId, "SELL", 30, 30);
    core.SubmitOrder(FrankBuyerId, "BUY", 30, 30);
   

    BOOST_CHECK_EQUAL(core.GetBalance(EveBuyerId), "USD: -30, RUB: 900\n");
    BOOST_CHECK_EQUAL(core.GetBalance(FrankBuyerId), "USD: 30, RUB: -900\n");

    core.ClearOrders();
}

BOOST_AUTO_TEST_CASE(TestMatchOrder4) 
{
    Core& core = GetCore();
    std::string EveBuyerId = core.RegisterNewUser("Eve");
    std::string FrankBuyerId = core.RegisterNewUser("Frank");
    ...
    core.SubmitOrder(EveBuyerId, "SELL", 5, 71);
    core.SubmitOrder(FrankBuyerId, "BUY", 30, 25);
    core.SubmitOrder(IvanBuyerId, "SELL", 22, 27);
	core.SubmitOrder(EveBuyerId, "BUY", 10, 50);
    core.SubmitOrder(AlekseiBuyerId, "SELL", 9, 20);
    core.SubmitOrder(AlenaSellerId, "BUY", 40, 100);


    BOOST_CHECK_EQUAL(core.GetBalance(EveBuyerId), "USD: 5, RUB: 85\n");
    BOOST_CHECK_EQUAL(core.GetBalance(FrankBuyerId), "USD: 9, RUB: -225\n");
	BOOST_CHECK_EQUAL(core.GetBalance(IvanBuyerId), "USD: -22, RUB: 594\n");
    BOOST_CHECK_EQUAL(core.GetBalance(AlekseiBuyerId), "USD: -9, RUB: 225\n");
    BOOST_CHECK_EQUAL(core.GetBalance(AlenaSellerId), "USD: 17, RUB: -679\n");
